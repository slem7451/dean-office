        ...
        $this->execute(<<<DB
            ...
            BEGIN
                IF (SELECT count(*) AS qty FROM student WHERE id = new.id) = 1 THEN
                    INSERT INTO student_history VALUES (new.id, new.first_name, new.second_name, new.patronymic, new.birthdate, new.created_at, new.closed_at, NOW(), 1);
                END IF;
                RETURN NULL;
            END
            $$
...
DB
        ...
        $this->execute(<<<DB
            CREATE OR REPLACE FUNCTION student_after_update()
                RETURNS trigger AS $$
            BEGIN
                INSERT INTO student_history VALUES (old.id, old.first_name, old.second_name, old.patronymic, old.birthdate, old.created_at, old.closed_at, NOW(), 2);
                RETURN NULL;
            END
            $$
            LANGUAGE 'plpgsql';
...
DB
        ...
        $this->execute(<<<DB
            CREATE OR REPLACE FUNCTION student_after_delete()
                RETURNS trigger AS $$
            BEGIN
                INSERT INTO student_history VALUES (old.id, old.first_name, old.second_name, old.patronymic, old.birthdate, old.created_at, old.closed_at, NOW(), 3);
                RETURN NULL;
            END
            $$
            LANGUAGE 'plpgsql';
